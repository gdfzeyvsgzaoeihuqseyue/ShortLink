...
export const useLinksStore = defineStore('links', () => {
  ...
  const createShortLink = async (longUrl: string): Promise<ShortLink | null> => {
    ...
    try {
      const response = await $fetch<CreateLinkResponse>(`${config.public.pgsBaseAPI}/shortlinks`, {
        method: 'POST',
        body: { longUrl }
      })
  ...
  const fetchLinks = async (page: number = 1, limit: number = 10): Promise<void> => {
    ...
    try {
      const response = await $fetch<GetLinksResponse>(`${config.public.pgsBaseAPI}/shortlinks`, {
        params: { page, limit }
      })
  ...
  const fetchLinkById = async (identifier: string): Promise<ShortLink | null> => {
    ...
    try {
      const response = await $fetch<GetLinkResponse>(`${config.public.pgsBaseAPI}/shortlinks/${identifier}`)
